...
describe('POST /api/playground/complexity', () => {
  let token: string;
...
class Rectangle:
    ...
    const res = await request(app)
      .post('/api/playground/complexity')
      .set('Authorization', `Bearer ${token}`)
      .send({ code, language: 'Python' });

    expect(res.status).toBe(200);
    expect(res.body.status).toBe('success');
    expect(res.body.message).toBe('Complexity checked successfully');
    expect(res.body.data.complexity.code).toContain('Time Complexity');
  });
